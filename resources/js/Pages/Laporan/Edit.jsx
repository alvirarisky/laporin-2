...
export default function Edit({ auth, laporan }) {
    ...
    const handleSaveSection = () => {
        ...
        router.put(route('sections.update', activeSection.id), {
        ...
        <AuthenticatedLayout user={auth.user} header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Editor: {laporan.judul}</h2>}>
            ...
            <div className="py-12">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    {flash && flash.success && (
                        <div className="mb-4 p-4 bg-green-100 text-green-700 rounded-lg shadow">{flash.success}</div>
                    )}
                                ...
                                <ul>
                                    {sections.map((section) => (
                                        <li key={section.id} className="mb-2">
                                            <button onClick={() => setActiveSection(section)} className={`w-full text-left p-2 rounded ${activeSection?.id === section.id ? 'bg-indigo-100 font-bold text-indigo-700' : 'hover:bg-gray-100'}`}>
                                                {section.title}
                                            </button>
                                        </li>
                                    ))}
                                ...
                                {activeSection ? (
                                    <>
                                        ...
                                        <div className="mt-4 flex items-center gap-4">
                                            <button onClick={handleSaveSection} className="px-4 py-2 bg-indigo-600 text-white font-semibold rounded-md disabled:bg-indigo-300 shadow hover:bg-indigo-700" disabled={isProcessing}>
                                                {isProcessing ? 'Menyimpan...' : 'Simpan Perubahan'}
                                            </button>
                                            
                                            <a href={route('laporan.preview', laporan.id)} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-gray-500 text-white font-semibold rounded-md shadow hover:bg-gray-600">
                                                Preview Laporan
                                            </a>

                                            <a href={route('laporan.download', laporan.id)} className="px-4 py-2 bg-green-600 text-white font-semibold rounded-md shadow hover:bg-green-700">
                                                Download (.docx)
                                            </a>
                                        </div>
                                    </>
                                    ...
                                    <div className="text-center p-10 h-full flex flex-col items-center justify-center text-gray-500">
                                        <p className="font-semibold text-lg">Pilih bab dari outline di kiri.</p>
                                    </div>
                                )}
                            </div>
                        </div>
