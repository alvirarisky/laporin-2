...
import axios from 'axios'; // Wajib import axios untuk AI
import QuestSidebar from '@/Components/QuestSidebar'; // Import Sidebar Questify
                    ...
                    <div className="w-[210mm] min-h-[297mm] bg-white text-black p-[2.54cm] shadow-xl origin-top scale-75 sm:scale-90 lg:scale-100 transition-transform duration-300 flex flex-col items-center text-center font-serif leading-relaxed">
                        ...
                        <div className="mb-14 flex-1 flex items-center justify-center w-full">
                            {logoUrl ? (
                                ...
                                <div className="w-32 h-32 border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400 text-xs text-center px-4">
                                    Logo Institusi Akan Muncul Di Sini
                                </div>
                            )}
                        </div>
...
export default function Create({ auth }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        logo: null,
        // Tambahkan sections kalau di backend kamu butuh array bab
        sections: [{ title: 'Pendahuluan', content: '' }]
    });

    const [showPreview, setShowPreview] = useState(false);

    // FUNGSI AI: Bantu bikin judul bray!
    const handleAiSuggestTitle = async () => {
        if (!data.mata_kuliah) {
            alert("Isi dulu Mata Kuliah-nya biar AI ada konteks, bray!");
            return;
        }
        ...
        try {
            const response = await axios.post(route('ai.generate'), {
                prompt: `Tolong buatkan 1 judul laporan ${data.report_type} yang keren dan formal untuk mata kuliah ${data.mata_kuliah}. Langsung berikan judulnya saja.`
            });
            setData('judul', response.data.result.replace(/"/g, '')); // Hapus tanda kutip jika ada
        } catch (error) {
            console.error(error);
            alert("Aduhhh AI-nya lagi pusing bray.");
        } finally {
            ...
            <div className="py-10">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <form onSubmit={submit} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        {/* KOLOM KIRI: FORM UTAMA (8 Kolom) */}
                        <div className="lg:col-span-8 space-y-6">
                            
                            {/* CARD 1: INFORMASI DASAR */}
                            <div className="bg-[#18181b] p-6 sm:p-8 rounded-2xl border border-zinc-800 shadow-xl transition-all hover:border-zinc-700">
                                ...
                                <div className="space-y-5">
                                    ...
                                    {/* Matkul (Pindah ke atas buat konteks AI) */}
                                    <div>
                                    ...
                                    <div>
                                        <div className="flex justify-between items-center mb-1.5">
                                            <label className="block text-sm font-medium text-zinc-400">Judul Laporan</label>
                                            <button 
                                                type="button" 
                                                onClick={handleAiSuggestTitle}
                                                disabled={isAiLoading}
                                                className="text-[10px] flex items-center gap-1.5 px-2 py-1 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-md hover:bg-indigo-500/20 transition-all uppercase tracking-wider font-bold"
                                            >
                                                <Icons.Sparkles /> {isAiLoading ? 'Mikir...' : 'Bikin Judul pakai AI'}
                                            </button>
                                        </div>
                                        <input
                                            type="text"
                            ...
                            <div className="bg-[#18181b] p-6 sm:p-8 rounded-2xl border border-zinc-800 shadow-xl transition-all hover:border-zinc-700">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div>
                                        ...
                                        <input type="text" value={data.nama} onChange={(e) => setData('nama', e.target.value)} className="w-full rounded-xl bg-zinc-900 border-zinc-700 text-white shadow-sm" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-zinc-400 mb-1.5">NIM / NISN</label>
                                        <input type="text" value={data.nim} onChange={(e) => setData('nim', e.target.value)} className="w-full rounded-xl bg-zinc-900 border-zinc-700 text-white shadow-sm" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-zinc-400 mb-1.5">Program Studi / Jurusan</label>
                                        <input type="text" value={data.prodi} onChange={(e) => setData('prodi', e.target.value)} className="w-full rounded-xl bg-zinc-900 border-zinc-700 text-white shadow-sm" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-zinc-400 mb-1.5">Kota Domisili Kampus</label>
                                        <input type="text" value={data.kota} onChange={(e) => setData('kota', e.target.value)} className="w-full rounded-xl bg-zinc-900 border-zinc-700 text-white shadow-sm" />
                                    </div>
                                    <div className="md:col-span-2">
                                        <label className="block text-sm font-medium text-zinc-400 mb-1.5">Dosen Pengampu / Pembimbing</label>
                                        <input type="text" value={data.dosen_pembimbing} onChange={(e) => setData('dosen_pembimbing', e.target.value)} className="w-full rounded-xl bg-zinc-900 border-zinc-700 text-white shadow-sm" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-4 space-y-6">
                            ...
                            {/* INTEGRASI QUESTIFY SIDEBAR (BARU) */}
                            <QuestSidebar formData={data} />
                            ...
                            <div className="bg-[#18181b] p-6 rounded-2xl border border-zinc-800 shadow-xl">
                                ...
                                <div className="mb-4 text-xs text-zinc-500 leading-relaxed bg-zinc-900/50 p-3 rounded-lg border border-zinc-800 italic">
                                    *Pastikan logo berformat PNG transparan agar cover terlihat profesional.
                                </div>
                                <div className="mt-2">
                                    <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-zinc-700 border-dashed rounded-xl cursor-pointer bg-zinc-900/50 hover:bg-zinc-800 hover:border-indigo-500 transition-all group">
                                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                            ...
                                            <p className="mb-2 text-sm text-zinc-400 group-hover:text-white transition-colors">
                                                <span className="font-semibold">Klik upload</span> logo
                                            </p>
                                        </div>
                                        <input type="file" className="hidden" accept="image/*" onChange={(e) => setData('logo', e.target.files[0])} />
                                    </label>
                                    {data.logo && <p className="mt-2 text-xs text-emerald-400 font-medium text-center">✓ {data.logo.name}</p>}
                                </div>
                            </div>

                            {/* CARD AKSI */}
                            <div className="bg-[#18181b] p-6 shadow-xl border border-zinc-800 rounded-2xl lg:sticky lg:top-24">
                                <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-4 border-b border-zinc-800 pb-2">Opsi Final</h3>
                                <div className="space-y-3">
                                    <button
                                        type="button"
                                        onClick={() => setShowPreview(true)}
                                        className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-zinc-800 border border-zinc-700 text-zinc-300 font-bold rounded-xl hover:bg-zinc-700 hover:text-white transition-all shadow-sm"
                                    >
